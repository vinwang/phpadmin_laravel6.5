...
<div class="left-nav">
    <div id="side-nav">
        <ul id="nav">
            ...
            @if($admin->hasPermissionTo($menu->id))
            <li>
                <a href="javascript:;">
                ...
                @if($menu->permissions)
                <ul class="sub-menu">
                    @foreach($menu->permissions as $sub)
                    @if($admin->hasPermissionTo($sub->id))
                    @if($sub->permissions->count())
                    <li>
                        <a href="javascript:;">
                            <i class="iconfont">
                                @if($sub->icon)
                        ...
                        <ul class="sub-menu">
                            @foreach($sub->permissions as $per)
                            @if($admin->hasPermissionTo($per->id))
                            <li>
                                <a onclick="xadmin.add_tab('{{ $per->name }}', '{{ route($per->uri) }}')">
                                    <i class="iconfont">
                                        @if($per->icon)
                                        {!! $per->icon !!}
                                        @else
                    ...
                    @else
                    <li>
                        <a onclick="xadmin.add_tab('{{ $sub->name }}','{{ route($sub->uri) }}')">
                            <i class="iconfont">
                            @if($sub->icon)
                            {!! $sub->icon !!}
                            @else
