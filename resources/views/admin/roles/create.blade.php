...
<div class="layui-fluid">
    <div class="layui-row">
        <form action="{{ route('admin.roles.store') }}" method="POST" class="layui-form layui-form-pane valideform">
        	{{ csrf_field() }}
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="layui-input-block">
                                    <input type="checkbox" name="permission_id[]" value="{{ $val->id }}" class="layui-hide per{{ $val->id }}" lay-ignore>

                                    @foreach($val->permissions as $v)

                                    @if($v->permissions->count())
                                    <input type="checkbox" name="permission_id[]" value="{{ $v->id }}" class="layui-hide  per{{ $v->id }}"lay-ignore>

                                    @foreach($v->permissions as $p)
                                    <input name="permission_id[]" lay-skin="primary" type="checkbox" title="{{ $p->name }}" value="{{ $p->id }}" pid="{{ $v->id }}" ppid="{{ $val->id }}" lay-filter="son" class="son">
                                    @endforeach
                                    @else
                                    <input name="permission_id[]" lay-skin="primary" type="checkbox" title="{{ $v->name }}" value="{{ $v->id }}" pid="{{ $val->id }}" lay-filter="son" class="son" @if(in_array($v->id, [18,19])) checked @endif>
                                    @endif
                                    @endforeach
                                </div>
                            </td>
            ...
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <textarea placeholder="请输入内容" id="desc" name="desc" class="layui-textarea">{{ old('desc') }}</textarea>
                </div>
            </div>
            <div class="layui-form-item">
