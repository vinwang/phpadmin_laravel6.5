;!function (win) {
  ...
  Xadmin.prototype.end = function() {
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh,isparent) {
		var id = md5(url);//md5每个url

		//重复点击
    var iframe = $('.x-iframe'), target;
    if(isparent == 1){
      target = parent;
    }
    if(isparent == 2){
    ...
    if(isparent) iframe = $('.x-iframe', target.document);

		for (var i = 0; i < iframe.length; i++) {
        if(iframe.eq(i).attr('tab-id')==id){
            if(isparent){
              target.element.tabChange('xbs_tab', id);
            }
            else{
		...
		this.add_lay_tab(title,url,id,target);
  ...
  Xadmin.prototype.del_tab = function (id) {
	...
	Xadmin.prototype.add_lay_tab = function(title,url,id,target) {
    if(target){
      target.element.tabAdd('xbs_tab', {
         title: title 
  ...
  Xadmin.prototype.father_reload = function() {
    ...
    if(typeof is_remember!="undefined")
		...
		return layui.data('tab_list'+userId)
	}
	/**
	 * [set_data 增加某一项]
  ...
  Xadmin.prototype.set_cate_data = function(data) {
	...
	Xadmin.prototype.del_data = function(id) {
		...
		if(typeof id!="undefined"){
			...
			layui.data('tab_list'+userId,null);
		}
	};
	/**
	 * [del_other_data 删除其它]
